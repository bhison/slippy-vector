...
public class SlippyTile : ScriptableObject //? ScriptableObjects are generally just for storing data
{
    ...
    //? Who calls this and how do they get a byte array
    //? Byte arrays are the standard way of transmitting binary data such as images
    public void SetRaster(byte[] data)
    {
        ...
        //? New empty texture which is 256x256 pixels
        var raster = new Texture2D(256, 256);
        raster.LoadImage(data); //? Loads the image by a byte array recieved through param
    ...
    {
        var rend = obj.GetComponent<MeshRenderer>();
        rend.enabled = --pendingTasks == 0; //reduces pending tasks by 1 then checks if its 0 and enables if so
    }
}
